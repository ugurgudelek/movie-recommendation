...
class MovieLens:
    ...
    def top_movies(self, n=5):
        top_n_movies_id = self.data[['itemID', 'rating']].groupby(by='itemID').mean().sort_values(by=['rating'], ascending=False).index.values[:n]
...
if __name__=="__main__":
    ...
    util_df=mv.data.pivot_table(values='rating',index='userID',columns='itemID')
